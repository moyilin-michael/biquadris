...
int main(int argc, char *argv[])
{
	...
	if (argc >= 2)
	{
		for (int start = 1; start < argc; start++)
		{
			...
			if (strcmp(argv[start], "-startlevel") == 0) {
				int llevel = stoi(argv[start + 1]);
				if (llevel <= 4 && llevel >= 0) {
					player1->level = llevel;
					player2->level = llevel;
				}
				start++;
			}
	...
	while (cin >> strans)
	{
		...
		{ 
			...
			if (numcount > 0) {
				string s = sinput.substr(0, numcount);
				//cout << s << endl;
				time = stoi(s);
				...
				if (sinput.length() >= 6)
				{
					if (sinput[5] == 'd')
					{
						for (int b = 0; b < time; ++b) {
							if (turn == 1)
						        {
							     if (player1->level - 1 >= 0) {
								     player1->level -= 1;
							     }
						        }
						        else
						        {
							     if (player2->level - 1 >= 0) {
								     player2->level -= 1;
							     }
						        }
						}
					...
					if (sinput[5] == 'u')
					{
						for (int c = 0; c < time; ++c) {
							if (turn == 1)
						        {
							    if (player1->level + 1 <= 4) {
								    player1->level += 1;
							    }
						        }
						        else
						        {
							    if (player2->level + 1 <= 4) {
								    player2->level += 1;
							    }
						        }
						}
